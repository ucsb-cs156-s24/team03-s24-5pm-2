...
public class UCSBDiningCommonsMenuItemIT {
    ...
    public void test_that_logged_in_user_can_get_by_id_when_the_id_exists() throws Exception {
            ...
            UCSBDiningCommonsMenuItems menuItems = UCSBDiningCommonsMenuItems.builder()
    ...
    public void an_admin_user_can_post_a_new_ucsbdiningcommonsmenuitem() throws Exception {
            ...
            UCSBDiningCommonsMenuItems menuItemsOne = UCSBDiningCommonsMenuItems.builder()
            .id(2)
            .name("ChickenSoup")
            ...
            ucsbDiningCommonsMenuItemsRepository.save(menuItemsOne);

            // act
            MvcResult response = mockMvc.perform(
                            post("/api/ucsbdiningcommonsmenuitem/post?name=ChickenSoup&station=EntreeSpecials&diningCommonsCode=dlg")
                                            .with(csrf()))
                            .andExpect(status().isOk()).andReturn();

            // assert
            String expectedJson = mapper.writeValueAsString(menuItemsOne);
            String responseString = response.getResponse().getContentAsString();
            assertEquals(expectedJson, responseString);
    }
}
