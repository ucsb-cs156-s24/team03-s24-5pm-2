...
describe("ArticlesIndexPage tests", () => {
    ...
    test("renders three articles correctly for regular user", async () => {
        ...
        const title = screen.getByText("Article 1 Title");
        expect(title).toBeInTheDocument();

        const url = screen.getByText("Article 1 URL");
        expect(url).toBeInTheDocument();

        const explanation = screen.getByText("Article 1 Explanation");
        expect(explanation).toBeInTheDocument();

        const email = screen.getByText("Article 1 Email");
        expect(email).toBeInTheDocument();

        const dateAdded = screen.getByText("2024-05-01T12:00:00");
